...
LocalSend Hub 采用 **双服务解耦架构**，核心文件接收服务与管理面板作为**独立进程**运行，通过共享配置文件和 SQLite 数据库通信，实现故障隔离。
...
```text
  ...
  - Config  - Device Identity                    │                     │
                   ...
                   └─────────────────────────────────────┘             │
                                                                       │
  关键优势: Admin 崩溃不影响文件接收，日志通过 SQLite 实时共享          │
└──────────────────────────────────────────────────────────────────────┘
```
...
└── localsend_logs.db*          # [忽略] SQLite 日志数据库
```
...
**日志管理**: 存储在 SQLite 数据库中 (`localsend_logs.db`)，支持并发读写（WAL 模式），超出 `MaxLogs`（默认 1000）自动清理最旧条目。两个服务共享同一数据库，确保日志一致性。

**会话管理**: `Sessions` map 存储 `sessionId → {fileId → fileName}`，仅存于核心服务内存，不持久化。

**配置持久化**:
- 核心服务每 15 秒定时保存 + 配置变更立即保存（JSON 配置文件）
- 管理服务在配置变更时立即保存，并轮询文件检测核心服务的变更
- 日志通过 SQLite 持久化，两个服务共享同一数据库
- Core 服务独占初始化数据库（WAL 模式、建表），Admin 服务只读打开，避免 SQLITE_BUSY 锁冲突
- Core 初始化失败不会中断服务，但日志会被跳过（需检查启动日志）
...
| 数据同步 | SQLite + JSON 文件轮询 | 日志实时一致，配置简单可靠 |
| SQLite 并发 | Core 独占写入 + Admin 只读打开 | 多进程避免 SQLITE_BUSY 锁冲突 |
| 证书管理 | 内存中生成 | 启动快，不依赖文件系统 |
| UI 方案 | `go:embed` + 响应式 Vanilla JS | 移动端自适应，无构建步骤 |
...
- Docker 支持环境变量覆盖配置
- SQLite 数据库路径可通过 `LOCALSEND_DB_PATH` 环境变量自定义
